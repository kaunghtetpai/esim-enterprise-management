...
export class SystemErrorService {
  ...
  private async checkSystemResources(): Promise<{ disk: number; memory: number }> {
    try {
      // Check disk space
      const { stdout: diskOutput } = await execAsync(
        'powershell -Command "Get-WmiObject -Class Win32_LogicalDisk | Where-Object {$_.DriveType -eq 3} | Select-Object Size,FreeSpace | ConvertTo-Json"'
      );
      ...
      const { stdout: memOutput } = await execAsync(
        'powershell -Command "Get-WmiObject -Class Win32_OperatingSystem | Select-Object TotalVisibleMemorySize,FreePhysicalMemory | ConvertTo-Json"'
      );
  ...
  private calculateOverallHealth(health: SystemHealth): 'healthy' | 'warning' | 'critical' {
  ...
  private logError(type: SystemError['type'], severity: SystemError['severity'], message: string, details: any) {
  ...
  private async attemptDatabaseFix(): Promise<boolean> {
    try {
  ...
  private async attemptAuthFix(): Promise<boolean> {
    try {
  ...
  async autoFixErrors(): Promise<{ fixed: number; failed: number }> {
    let fixed = 0;
    let failed = 0;
  ...
  async getErrorReport(): Promise<SystemError[]> {
