...
interface EnterpriseSetupConfig {
  tenantId: string;
  adminAccount: string;
  carriers: {
    name: string;
...
export class EnterpriseSetupService {
  private supabase = createClient(
    process.env.SUPABASE_URL!,
    process.env.SUPABASE_ANON_KEY!
  );

  private config: EnterpriseSetupConfig = {
  ...
  async runCompleteSetup(): Promise<{ success: boolean; phases: SetupPhase[]; summary: any }> {
    try {
  ...
  async validateCurrentSetup(): Promise<{ valid: boolean; issues: string[]; recommendations: string[] }> {
    try {
      const scriptPath = 'scripts\\Complete-eSIM-Enterprise-Setup.ps1';
  ...
  async getSetupStatus(): Promise<SetupPhase[]> {
    return this.phases;
  }

  async runPhase(phaseNumber: number): Promise<SetupPhase> {
  ...
  async createCompliancePolicies(): Promise<{ created: string[]; existing: string[]; errors: string[] }> {
    const results = { created: [], existing: [], errors: [] };
    ...
    try {
      ...
      const existingPolicy = policies.value?.find((p: any) => p.displayName === policyName);
  ...
  async configureCompanyPortal(): Promise<{ success: boolean; message: string }> {
    try {
  ...
  private parseSetupResults(output: string): { success: boolean; summary: any } {
  ...
  private parseValidationResults(output: string): { valid: boolean; issues: string[]; recommendations: string[] } {
    const lines = output.split('\n');
    const issues = [];
  ...
  private extractPhaseFromLine(line: string): SetupPhase | null {
  ...
  private async storeSetupResults(results: any): Promise<void> {
    try {
      await this.supabase
...
export const enterpriseSetupService = new EnterpriseSetupService();