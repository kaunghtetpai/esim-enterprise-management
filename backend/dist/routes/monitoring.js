...
router.get('/health/detailed', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
        const [errorMetrics, healthMetrics, apiMetrics, systemConfig] = await Promise.all([
            (0, database_1.query)(`
        SELECT 
      ...
      `),
        ...
        WHERE timestamp >= NOW() - INTERVAL '1 hour'
      `),
            (0, database_1.query)('SELECT config_key, config_value FROM system_configuration')
        ]);
        const errorHandler = comprehensiveErrorHandler_1.ComprehensiveErrorHandler.getMetrics();
        res.json({
            success: true,
            data: {
...
router.get('/errors/recent', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
...
router.get('/performance/metrics', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
...
router.post('/alerts/configure', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
...
router.get('/system/status', async (req, res) => {
    try {
        ...
        try {
            const errorCheck = await (0, database_1.query)(`
