...
// Create new profile
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'it_staff']), validation_1.validateProfile, async (req, res) => {
    try {
        ...
        const { iccid, profile_name, carrier, carrier_code, msisdn, plan_name, data_allowance_mb, validity_days, department_id, cost_center, monthly_cost } = req.body;
...
router.put('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin', 'it_staff']), async (req, res) => {
    try {
        ...
        const currentResult = await (0, database_1.query)('SELECT * FROM esim_profiles WHERE id = $1 AND organization_id = $2', [id, organizationId]);
...
router.delete('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), async (req, res) => {
    try {
        ...
        const profileCheck = await (0, database_1.query)('SELECT * FROM esim_profiles WHERE id = $1 AND organization_id = $2', [id, organizationId]);
        ...
        const assignmentCheck = await (0, database_1.query)('SELECT COUNT(*) FROM device_profile_assignments WHERE profile_id = $1 AND status = $2', [id, 'active']);
        ...
        const result = await (0, database_1.query)('DELETE FROM esim_profiles WHERE id = $1 AND organization_id = $2 RETURNING *', [id, organizationId]);
        // Log audit trail
        try {
            await (0, database_1.query)(`
